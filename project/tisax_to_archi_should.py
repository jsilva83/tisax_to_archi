...
REQUIREMENTS_FILE = 'C:/Users/jorge.silva/Downloads/tisax_should_requirements.xlsx'


def tisax_to_archi_should():

    # Create en empty output dataframe.
    df_data = pd.DataFrame(columns=['objectives', 'groups', 'should_requirements', 'should_sub-requirements'])
    df_objectives = pd.DataFrame(columns=['objectives'])
    df_groups = pd.DataFrame(columns=['groups'])
    df_should_reqs = pd.DataFrame(columns=['should_requirements'])
    df_should_subs = pd.DataFrame(columns=['should_sub-requirements'])

    # Get "should" requirements from TISAX check list.
    ...
    for a_n in range(5, a_xls.last_row + 1):
        ...
        # ]
        a_row = a_xls.get_should_row(a_n)

        if len(a_row) > 0:

            objective = a_row[0]
            objective_nr = objective.split(' ')[0]
            group = f'{objective_nr} Should'
            requirement = ''
            requirement_nr = ''
            index_r = 0
            index_s = 0
            ...
            for item in a_row[1]:
                ...
                if len(item_list) > 1:
                    ...
                    df_req = pd.DataFrame({'should_requirements': [requirement]})
                    df_should_reqs = pd.concat([df_should_reqs, df_req], ignore_index=True)
                ...
                if len(item_list) > 1:
                    ...
                    df_sub_req = pd.DataFrame({'should_sub-requirements': [sub_requirement]})
                    df_should_subs = pd.concat([df_should_subs, df_sub_req], ignore_index=True)
                ...
                df_row_to_add = pd.DataFrame({'objectives': [objective], 'groups': [group],
                                              'should_requirements': [requirement],
                                              'should_sub-requirements': [sub_requirement]})
    ...
    df_should_reqs.to_excel(a_excel_to_write, sheet_name='should_requirements', index=False)
    df_should_subs.to_excel(a_excel_to_write, sheet_name='should_sub-requirements', index=False)
    a_excel_to_write.save()
    return


if __name__ == '__main__':
    tisax_to_archi_should()
